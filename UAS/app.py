...
def admin_index():
    ...
    sql = """
        SELECT tb.*, u.nama_ukuran
        FROM toko_baju tb
        LEFT JOIN ukuran u ON tb.ukuran_id = u.id
        WHERE 1=1
    """
    params = []

    if q:
        sql += " AND (tb.kode_baju LIKE %s OR tb.nama_baju LIKE %s)"
        params += [f"%{q}%", f"%{q}%"]

    if size:
        sql += " AND tb.ukuran_id = %s"
        params.append(size)

    cursor.execute(sql, params)
    dtstok = cursor.fetchall()

    cursor.execute("SELECT * FROM ukuran ORDER BY nama_ukuran")
    sizes = cursor.fetchall()

    return render_template("ADMIN/index.html", dtstok=dtstok, sizes=sizes)

# admin ukuran
@app.route("/admin/ukuran", methods=["GET", "POST"])
def ukuran():

    if request.method == "POST":
        nama = request.form["nama_ukuran"]
        cursor.execute(
    ...
    cursor.execute("SELECT * FROM ukuran ORDER BY nama_ukuran")
    data_ukuran = cursor.fetchall()

    return render_template("ADMIN/ukuran.html", data_ukuran=data_ukuran)


@app.route("/admin/ukuran/delete/<int:id>")
def delete_ukuran(id):
    cursor.execute("DELETE FROM ukuran WHERE id=%s", (id,))
    db.commit()
    return redirect(url_for("ukuran"))
...
def add():
    ...
    if request.method == "POST":
        ...
        ukuran_id = request.form["ukuran_id"]
        ...
        cursor.execute("""
            ...
            (kode_baju, nama_baju, ukuran_id, warna, stok, harga, filename, kategori)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
        """, (kode, nama, ukuran_id, warna, stok, harga, filename, kategori))

        db.commit()
        return redirect(url_for("admin_index"))

    return render_template("ADMIN/add.html", sizes=sizes)
...
def admin_edit(kode_baju):
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            ...
            SET nama_baju=%s,
                ...
                kategori=%s
            WHERE kode_baju=%s
        """, (nama, ukuran_id, warna, stok, harga, kategori, kode_baju))
    ...
    cursor.execute("""
        ...
        FROM toko_baju tb
        LEFT JOIN ukuran u ON tb.ukuran_id = u.id
        WHERE tb.kode_baju=%s
    """, (kode_baju,))
    barang = cursor.fetchone()

    return render_template("ADMIN/edit.html", barang=barang, sizes=sizes)
